  ...
  return (
    <div className="min-w-0 min-h-[calc(100vh-7rem)] overflow-x-hidden bg-[#F4F7FB]">
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        <p className="text-xs text-[#64748B]">Total: {visible.length} cuentas</p>
        <div className="flex items-center gap-1 lg:hidden">
          <button
            type="button"
            className="rounded-md border border-[#E2E8F0] bg-white px-2 py-1 text-[11px] font-semibold text-[#0F172A]"
            onClick={() => {
          ...
          <button
            type="button"
            className="rounded-md border border-[#E2E8F0] bg-white px-2 py-1 text-sm text-[#0F172A]"
            aria-label="Alejar para ver todas las columnas"
            onClick={() => changeZoom(zoom - 0.08)}
          >
            −
          </button>
          <span className="min-w-12 text-center text-[11px] text-[#64748B]">{Math.round(zoom * 100)}%</span>
          <button
            type="button"
            className="rounded-md border border-[#E2E8F0] bg-white px-2 py-1 text-sm text-[#0F172A]"
      ...
      <p className="mt-1 text-[11px] text-[#64748B] lg:hidden">
              ...
              return (
                <article key={cuenta.id} className="overflow-hidden rounded-xl border border-[#E8EEF6] bg-white">
                  <div className={`${headerGrid} px-0 hover:bg-[#F8FAFC]`}>
                    ...
                    <div className="px-2 py-3">
                      <span className="inline-flex rounded-full bg-[#DBEAFE] px-2.5 py-0.5 text-[11px] font-semibold text-[#2563EB]">
                    ...
                    <div className="relative flex items-center justify-end gap-0.5 px-2 py-2">
                      ...
                      <button
                        type="button"
                        className="ml-1 inline-flex h-8 w-8 items-center justify-center rounded-md border border-[#E2E8F0] text-[#0F172A] hover:border-[#2563EB]"
                        aria-expanded={open}
                      ...
                      {menu === cuenta.id ? (
                        <div className="absolute top-10 right-2 z-20 w-40 rounded-lg border border-[#E8EEF6] bg-white py-1 text-xs text-[#0F172A] shadow-sm">
                          <button type="button" className="block w-full px-3 py-2 text-left hover:bg-[#F8FAFC]" onClick={() => { setDetail(cuenta.id); setMenu(null); }}>
                            Ver detalle
                          </button>
                        </div>
                      ) : null}
                  ...
                  {open ? (
                    <div className="border-t border-[#E8EEF6] bg-[#F8FAFC] py-3 pr-3 pl-12">
                        ...
                        <tbody>
                          {perfilesVisibles.map((perfil) => {
                            ...
                            const perfilWhatsapp = perfil.clienteTelefono
                              ...
                              : null;
                            return (
                              <tr key={perfil.id} className="border-t border-[#E8EEF6] text-[#0F172A]">
                                <td className="px-2 py-2">{etiquetaCliente(perfil)}</td>
                                <td className="px-2 py-2">{perfil.pin ? perfil.pin : <span className="text-[#94A3B8] italic">Sin PIN</span>}</td>
                                ...
                                <td className="px-2 py-2">
                                  <HealthStatusBadge status={perfilHealth} look="soft" />
                                </td>
                                <td className="relative px-2 py-2">
                                  <div className="flex items-center gap-0.5">
                                    <WhatsAppMenu destino={perfilWhatsapp} plantillas={plantillas} tone="light" />
                                    <IconBtn label="Ver" onClick={() => setDetail(perfil.id)} tone="light">
                                      <EyeIcon className="h-3.5 w-3.5" />
                                    </IconBtn>
                                    <IconBtn label="Editar" onClick={() => setDetail(perfil.id)} tone="light">
                                      <PencilIcon className="h-3.5 w-3.5" />
                                    </IconBtn>
                                    <IconBtn label="Más" onClick={() => setMenu(menu === menuKey ? null : menuKey)} tone="light">
...
function etiquetaCliente(perfil: Perfil) {
  if (!perfil.clienteTelefono) {
    return (
      <span className="inline-flex items-center gap-1.5 font-medium text-[#EAB308]">
