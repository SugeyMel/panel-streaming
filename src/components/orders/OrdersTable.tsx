  ...
  return (
    <DataTable
      ...
      columns={[
        ...
        { key: "whatsapp", header: "WhatsApp", render: (order: (typeof rows)[number]) => order.whatsapp },
        ...
        { key: "date", header: "Fecha", render: (order: (typeof rows)[number]) => formatDate(order.createdAt) },
        {
          key: "action",
          header: "Acciones",
          render: (order: (typeof rows)[number]) => (
            <Button
              href={actionHref ? actionHref(order) : undefined}
              variant="ghost"
